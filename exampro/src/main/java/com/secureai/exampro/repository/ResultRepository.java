...
public interface ResultRepository 
    ...
    Optional<Result> findBySession(
            ExamSession session
    );

    // Check if result already exists
    boolean existsBySession(
            ExamSession session
    );

    // Find all results by status (PASS/FAIL)
    List<Result> findByResultStatus(
            ResultStatus resultStatus
    );

    // Find results by percentage greater than equal
    List<Result> findByPercentageGreaterThanEqual(
            Double percentage
    );
